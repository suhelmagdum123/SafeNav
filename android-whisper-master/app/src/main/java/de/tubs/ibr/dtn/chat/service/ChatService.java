...
import de.tubs.ibr.dtn.chat.PeerActivity;
...
public class ChatService extends DTNIntentService {
	...
	private DataHandler _data_handler = new DataHandler()
	{
		...
		private void eventNewPresence(SingletonEndpoint source, Date created, String payload, Long flags)
		{
            ...
            //first part ::Latitude
            double my_lat = Double.parseDouble(parts[0]);
            //second part ::Longitude
            double my_lon = Double.parseDouble(parts[1]);
            double distance = -1.0000;
            Location startPoint=new Location("myLocation");
            startPoint.setLatitude(my_lat);
            startPoint.setLongitude(my_lon);

            Location endPoint=new Location("buddyLocation");
			...
			while (tokenizer.hasMoreTokens())
			{
				...
				{
                    presence = value;
                    //get the latitude and longitude from  presence
                    final int b_mid = presence.length() / 2;//get the middle of the String
                    String[] b_parts = {presence.substring(0, b_mid),presence.substring(b_mid)};
                    //first part ::Latitude
                    double b_lat = Double.parseDouble(b_parts[0]);
                    //second part ::Longitude
                    double b_lon = Double.parseDouble(b_parts[1]);

                    endPoint.setLatitude(b_lat);
                    endPoint.setLongitude(b_lon);
				...
				else if (keyword.equalsIgnoreCase("Status"))
				{
                    ...
                    status += "Distance in meters :"+Double.toString(distance);
				}
	...
	private void showNotification(Intent intent)
	{
		...
		intent.setClass(this, PeerActivity.class);
	...
	private void actionSendMessage(Long buddyId, String text) {
		try {
			...
			Cursor c = getRoster().queryBuddies();
			RosterAdapter.ColumnsMap cmap = new RosterAdapter.ColumnsMap();
			while (c.moveToNext())
			{ Buddy buddy = new Buddy(this, c, cmap);
				...
				//android.os.Message msg = mUpdateHandler.obtainMessage(REFRESH_BUDDY_DATA, b.getEndpoint());
				// schedule the update in 61 minutes
				//mUpdateHandler.sendMessageDelayed(msg, 61 * 60 * 1000); Log.d(TAG, "scheduled update for " + b.getEndpoint());
